...
minetest.register_on_generated(function(minp, maxp)
	...
	if not schematic_placed then
		if ORIGIN.x >= minp.x and ORIGIN.x <= maxp.x
			...
			gamelog.event("ARENA_PLACED", { arena = 1, at = ORIGIN })
		end
	end
	...
	if not schem2_placed then
		if ORIGIN2.x >= minp.x and ORIGIN2.x <= maxp.x
			...
			schem2_placed = true
			gamelog.event("ARENA_PLACED", { arena = 2, at = ORIGIN2 })
		end
	end
end)
...
minetest.register_on_newplayer(function(player)
	...
	gamelog.event("PLAYER_INIT",
		{ player = player:get_player_name(), spawn = SPAWN_POS, coins = 20 })
end)
...
minetest.register_on_joinplayer(function(player)
	...
	minetest.after(0.5, function()
		...
		if p then
			p:set_pos(SPAWN_POS)
			gamelog.event("TELEPORT",
				{ player = pname, to = SPAWN_POS, reason = "join" }, p)
		end
	end)
end)
...
minetest.register_on_respawnplayer(function(player)
	player:set_pos(SPAWN_POS)
	gamelog.event("TELEPORT",
		{ player = player:get_player_name(), to = SPAWN_POS, reason = "respawn" }, player)
	return true
end)

-- Auto-place arena 2 on every server start using emerge_area so mapblocks exist first
--
-- after(0): emerge_area mag niet tijdens script-init worden aangeroepen. Deed
-- je dat wel, dan weigerde de engine de aanroep ("Calling this function during
-- script init is disallowed") en werd arena 2 bij het opstarten nooit geplaatst.
		...
		minetest.emerge_area(minp, maxp, function(blockpos, action, calls_remaining)
			if calls_remaining == 0 then
				minetest.after(0.1, function()
					minetest.place_schematic(ORIGIN2, SCHEM2_PATH, "0", nil, true)
					schem2_placed = true
					gamelog.event("ARENA_PLACED", { arena = 2, at = ORIGIN2, via = "emerge" })
				end)
			end
		end)
	end)
...
minetest.register_chatcommand("save_arena", {
	...
	func = function(name, param)
		...
		gamelog.event("ADMIN_SAVE_ARENA",
			{ player = name, from = minp, to = maxp, file = filepath })
